...
export const AuthModal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    await new Promise(r => setTimeout(r, 600)); // Smooth authenticating feedback
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    ...
    const demoEmail = provider === 'google' ? 'creator.alex@gmail.com' : 'dev.sarah@github.com';
    const demoName = provider === 'google' ? 'Alex Rivera' : 'Sarah Chen';
    
    login(demoEmail, demoName, provider);
    setIsLoading(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden relative">
        
        {/* Top Decorative Banner */}
        <div className="p-6 bg-gradient-to-tr from-primary-600 via-indigo-600 to-purple-600 text-white relative">
          <button
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <button
              ...
              disabled={isLoading}
              className="px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 rounded-xl text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500"
                    required
                  />
                </div>
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-2 rounded-xl text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 font-mono"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="p-1 text-slate-400 hover:text-slate-200 absolute right-2.5 top-2"
                >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-2.5 rounded-xl text-xs font-bold bg-primary-600 hover:bg-primary-500 text-white flex items-center justify-center gap-2 shadow-md shadow-primary-500/20 transition-all disabled:opacity-50 mt-2"
            >
              {isLoading ? (
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 text-center">
            <button
              ...
              className="text-xs text-slate-500 dark:text-slate-400 hover:text-primary-500 flex items-center justify-center gap-1.5 mx-auto"
            >
