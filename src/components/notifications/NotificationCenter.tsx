    ...
    <div className="relative" ref={trayRef}>
      ...
      {isOpen && (
        <>
          {/* Backdrop on mobile */}
          <div
            className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-xs sm:hidden"
            onClick={() => setIsOpen(false)}
          />

          <div className="fixed left-3 right-3 top-16 sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-96 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-4 z-50 animate-fadeIn space-y-3 max-h-[85vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-1 pb-2 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                ...
                {unreadNotifsCount > 0 && (
                  <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-primary-100 text-primary-600 dark:bg-primary-950 dark:text-primary-400">
            ...
            {/* Notifications List */}
            <div className="max-h-64 sm:max-h-72 overflow-y-auto space-y-2 pr-1 divide-y divide-slate-100 dark:divide-slate-800/60">
                ...
                filteredNotifs.map((n) => (
                  <div
                    ...
                    className={`p-3 rounded-2xl cursor-pointer transition-all ${
                      !n.read 
                        ? 'bg-primary-50/70 dark:bg-primary-950/40 hover:bg-primary-100/70 dark:hover:bg-primary-950/60' 
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        ...
                        {n.actionLabel && (
                          <div className="pt-1">
                            <span className="text-[11px] font-bold text-primary-500 hover:underline flex items-center gap-1">
            ...
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 text-center">
              <button
                ...
                className="w-full py-2.5 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
              >
                <Sparkles className="w-3.5 h-3.5 text-primary-500" />
                <span>What's New in Voxaro Studio</span>
              </button>
            </div>
